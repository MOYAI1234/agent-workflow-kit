...
| 新增大特性（跨多文件） | 以上全部 + `docs/architecture.md` 新增章节 + `docs/handoff.md` 已完成清单 |
...
| 新增 Skill | CLAUDE.md 可用工具章节 · skill.json · SKILL.md |
| 新增脚本 | CLAUDE.md 脚本说明 · scripts/ 目录 README（如有） |
...
|------|---------|
| 过期事实 | 改记忆文件，同时更新索引 |
| 相对时间（"今天"、"最近"） | 全部转成绝对日期（`2026-05-27` 而非"今天"） |
| 重复记录（多条说同一件事） | 合并为一条 |
| 已完成的待办 | 删除——知识库不是历史档案 |
| 推翻的决策 | 删除旧条目，留新决策 |
| 跨会话只用一次的临时上下文 | 删除 |
